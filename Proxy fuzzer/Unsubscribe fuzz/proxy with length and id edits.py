...
def write_to_file_full_buffer(buffer):
    with open("buffer_log_proxy2.csv", 'a', encoding="utf-8", newline="") as csvfile:
...
def request_fuzz(buffer):
    ...
    for case in cases:
        ...
        if case in buffer:
            caseLen = len(case)
            newLength = random.randint(1, 400)

            # print("caseLen " + str(caseLen))
            # print("newLen " + str(newLength))
            # print(buffer.replace(caseLen.to_bytes(numSize, "big")+case, newLength.to_bytes(numSize, "big")+case))
            newBuffer += buffer.replace(caseLen.to_bytes(numSize, "big")+case, newLength.to_bytes(numSize, "big")+case)
